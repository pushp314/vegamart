  ...
  async getMonthlySettlementStatement(vendorId: string, monthStr?: string) {
    ...
    return {
      ...
      vendor: {
        ...
        bank_account_number: (vendor as any).bank_account_number ? `XXXX${(vendor as any).bank_account_number.slice(-4)}` : null,
        bank_ifsc: (vendor as any).bank_ifsc || null,
        bank_name: (vendor as any).bank_name || null,
        upi_id: (vendor as any).upi_id || null,
      },
      metrics: {
