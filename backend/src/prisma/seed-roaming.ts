...
async function main() {
  ...
  for (const v of vendors) {
    ...
    if (!profile) {
      ...
      console.log(`Created roaming vendor: ${v.name}`);
    } else {
      await prisma.vendorProfile.update({
        where: { id: profile.id },
        data: { roaming: true, is_open: true, latitude: v.lat, longitude: v.lng, status: "APPROVED" }
      });
      console.log(`Updated roaming vendor: ${v.name}`);
    }
  }

  console.log("Done seeding roaming vendors.");
}
