...
export function AdminDelivery({ deliveryList, onApprove, onReject, isApproving, isRejecting }: AdminDeliveryProps) {
  ...
  const approvedCount = deliveryList.filter((p) => (p.status || "").toLowerCase() === "approved").length;
  const pendingCount = deliveryList.filter((p) => (p.status || "").toLowerCase() === "pending").length;
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-3xl border border-border bg-card p-5 shadow-soft">
          <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Fleet</div>
          <div className="text-2xl font-black font-display text-foreground mt-1">{deliveryList.length}</div>
        </div>
        <div className="rounded-3xl border border-border bg-card p-5 shadow-soft flex items-center justify-between">
          <div>
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Active Riders</div>
            <div className="text-2xl font-black font-display text-emerald-600 mt-1">{approvedCount}</div>
          </div>
          <ShieldCheck className="h-8 w-8 text-emerald-500/40" />
        </div>
        <div className="rounded-3xl border border-border bg-card p-5 shadow-soft flex items-center justify-between">
          <div>
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Pending Review</div>
            <div className="text-2xl font-black font-display text-amber-600 mt-1">{pendingCount}</div>
          </div>
                ...
                <tr key={partner.id} className="hover:bg-muted/50 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-black uppercase tracking-wider border
                      ${status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-400 dark:border-emerald-500/30' :
                        status === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/15 dark:text-amber-400 dark:border-amber-500/30' :
                        status === 'suspended' ? 'bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-500/15 dark:text-violet-400 dark:border-violet-500/30' :
