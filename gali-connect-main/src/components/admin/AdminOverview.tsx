      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Monitor your marketplace performance and growth in real-time.
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {cards.map((c, i) => {
          ...
          return (
            <div
              key={i}
              className="group relative rounded-3xl bg-card p-6 shadow-soft border border-border hover:shadow-glow transition-shadow overflow-hidden"
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 ${c.glow} rounded-full blur-3xl -mr-10 -mt-10 transition-all group-hover:scale-150 duration-500`}
              />
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  ...
                  <h3 className="font-display text-4xl font-black mt-2 text-foreground">
                    {c.value}
                  </h3>
                </div>
              ...
              <div className="mt-6 flex items-center gap-2 relative z-10">
                <span
                  className={`flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${c.up ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-rose-50 text-rose-600 border border-rose-100"}`}
                >
                  {c.up ? (
              ...
              {c.chart && c.chart.length > 0 && (
                <div className="mt-4 h-10 w-full relative z-10 opacity-90">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      data={c.chart.map((v: number, i: number) => ({ v, i }))}
